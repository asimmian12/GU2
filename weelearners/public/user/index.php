...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_FILES['profile_picture']['name'])) {
    ...
    if (in_array($fileType, $allowTypes)) {   
        // By creating an upload directory if it doesn't already exist
        if (!is_dir($targetDir)) {
            mkdir($targetDir, 0755, true);
        }
        // By moving uploaded file and update database
        if (move_uploaded_file($_FILES['profile_picture']['tmp_name'], $targetFilePath)) {           
            ...
            if ($accounts->execute()) {
                ...
                // By refreshing the page to show the new image
                header("Refresh:0");
            } else {
...
// The database query to fetch account details
$userId = $_SESSION['id']; 
$sql = "SELECT 
    id, 
    username, 
...
// The database query to fetch user details
$sql = "SELECT 
    username, 
    email, 
    is_active, 
        ...
        <section class="bg-white w-full p-6 rounded-lg shadow-md">
            <div class="space-y-4 text-center">
                <?php if (isset($userPicture) && !empty($userPicture)): ?>
                    <!-- To display user profile picture if its available -->
